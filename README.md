...
## start-mac-agents.sh
Mac環境で、複数のターミナルウィンドウを自動で開いてエージェントを一括起動できる補助スクリプト。
...
- president用・multiagent用の2つのターミナルウィンドウが自動で開き、それぞれのtmuxセッションに接続されます。
- Claudeのワーキングディレクトリは常にClaude-Code-Communication直下になります。
- shを実行した元のターミナルウィンドウは自動で閉じます。
- 公式READMEの手順が基本ですが、Macユーザーはこのスクリプトでより簡単に起動できます。

> ※Mac専用です。Linuxでは動作しません。
